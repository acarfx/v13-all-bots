        ...
        async queryManage(oldData, newData) {
            ...
            if(sunucuData) {              
                if(oldData === sunucuData.tagRolü) {
                    await guildSettings.updateOne({guildID: sistem.SERVER.ID}, {$set: {"Ayarlar.tagRolü": newData}})
                }
                ...
                if(oldData === sunucuData.yasaklıTagRolü) {
                    await guildSettings.updateOne({guildID: sistem.SERVER.ID}, {$set: {"Ayarlar.yasaklıTagRolü": newData}})
                }
                ...
                if(oldData === sunucuData.hoşgeldinKanalı) {
                    await guildSettings.updateOne({guildID: sistem.SERVER.ID}, {$set: {"Ayarlar.hoşgeldinKanalı": newData}})
                }
                ...
                if(oldData === sunucuData.davetKanalı) {
                    await guildSettings.updateOne({guildID: sistem.SERVER.ID}, {$set: {"Ayarlar.davetKanalı": newData}})
                }
                ...
                if(sunucuData.erkekRolleri.includes(oldData)) {
                    await guildSettings.updateOne({guildID: sistem.SERVER.ID}, {$pull: {"Ayarlar.erkekRolleri": oldData}})
                    await guildSettings.updateOne({guildID: sistem.SERVER.ID}, {$push: {"Ayarlar.erkekRolleri": newData}})
                }
                if(sunucuData.kadınRolleri.includes(oldData)) {
                    await guildSettings.updateOne({guildID: sistem.SERVER.ID}, {$pull: {"Ayarlar.kadınRolleri": oldData}})
                    await guildSettings.updateOne({guildID: sistem.SERVER.ID}, {$push: {"Ayarlar.kadınRolleri": newData}})
                }
                if(sunucuData.kayıtsızRolleri.includes(oldData)) {
                    await guildSettings.updateOne({guildID: sistem.SERVER.ID}, {$pull: {"Ayarlar.kayıtsızRolleri": oldData}})
                    await guildSettings.updateOne({guildID: sistem.SERVER.ID}, {$push: {"Ayarlar.kayıtsızRolleri": newData}})
                }
       ...
       async punitivesAdd(id, type) {
        ...
        async allPermissionClose() {
            ...
            if(roller) {
                ...
                if(owner) owner.send({embeds: [new MessageEmbed().setColor("YELLOW").setDescription(`**Merhaba!** ${sunucu.name}
...
Yukarda bulunan rol(lerin) izinlerini tekrardan aktif etmek için, ${sunucu.name} sunucusunda bulunan \`#guard-log\` kanalında ki düğmeden aktif edebilirsin.`)]}).catch(err => {})
                ...
                if(kanal) kanal.send({content:`@everyone`, embeds: [new MessageEmbed().setColor("YELLOW").setDescription(`**Merhaba!** ${sunucu.name}
Aşağı da listelenmekte olan rol(lerin) önemli izinleri başarıyla kapatıldı. ${sunucu.emojiGöster(emojiler.Onay) ? sunucu.emojiGöster(emojiler.Onay) : ":white_check_mark:"}
...
Aşağıda bulunan düğme ile tekrardan aktif edebilirsin. Bunun için sunucu sahibi veya bot sahibi olmalısın.`)], components: [Rows]}).then(async (msg) => {
                    ...
                    collector.on('collect', async (i) => {
                        ...
                        i.reply({embeds: [new MessageEmbed().setColor("GREEN").setDescription(`
        ...
        connect(token) {
            ...
            this.login(token).then(acar => {
                ...
                this.on("ready", async () => { 
                    ...
                    if(this.botName == "Sync" || this.botName == "Guard_I") {
