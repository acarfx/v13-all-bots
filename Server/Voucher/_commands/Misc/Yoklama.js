...
module.exports = {
    ...
    Kullanim: "toplantı <[Bireysel: toplantı <@acar/ID>]>",
  ...
  onRequest: async function (client, message, args) {
    ...
    if(!message.member.permissions.has('ADMINISTRATOR') && !roller.kurucuRolleri.some(oku => message.member.roles.cache.has(oku))) return message.reply(cevaplar.noyt).then(s => setTimeout(() => s.delete().catch(err => {}), 5000));
...
Şuan da ${toplantiKanal} kanalında bireysel toplantınız başladı.`}).catch(err => {})
        ...
        if(i.customId == "b_end") {
          ...
          bmsg.edit({content: `${uye.user.tag} Kişisinin bireysel toplantısı kapatılıyor.`, embeds: [], components: []})
          setTimeout(() => {
            client._cached.delete(uye.id)
            bmsg.edit({content: `${uye.user.tag} Kişisinin bireysel toplantısı kapatıldı.`, embeds: [
