...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    if(uye) {
         if(!uye.voice.channel) return message.reply(`${message.guild.emojiGöster(emojiler.Iptal)} Belirtilen **${uye.user.tag}** isimli üye seste aktif olmadığından işlem iptal edildi.`).then(x => {
            message.react(message.guild.emojiGöster(emojiler.Iptal) ? message.guild.emojiGöster(emojiler.Iptal).id : undefined)
            setTimeout(() => {
                x.delete()
            }, 8500);
