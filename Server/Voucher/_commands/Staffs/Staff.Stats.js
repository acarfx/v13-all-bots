...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    let barSystem = [
      ...
      {id: "diger", başlamaBar: "acar_baslangicbar", doluBar: "acar_dolubar", doluBitişBar: "acar_dolubitisbar", boşBitişBar:"acar_bosbitis", boşBar: "acar_bosbar", başlangıçBar: "acar_bosbaslama"},
    ]
    ...
    Stats.findOne({ guildID: message.guild.id, userID: uye.id }, async (err, data) => {
      if(!puanBilgisi) return message.reply(cevaplar.data).then(s => setTimeout(() => s.delete().catch(err => {}), 5000));
        ...
        siralamabul.filter(x => message.guild.members.cache.has(x._id)).sort((a, b) => b.ToplamPuan - a.ToplamPuan).forEach((veri, index) => {
         ...
         let embed = new genEmbed().setAuthor(uye.user.tag, uye.user.avatarURL({dynamic: true, size: 2048})).setThumbnail(uye.user.avatarURL({dynamic: true, size: 2048})).setDescription(`${uye} (${uye.roles.highest}) kullanıcısının yetki ve yükseltim bilgileri aşağıda belirtilmiştir.`)
        .addFields(
...
{ name: `Yetki Durumu`, value: yetkidurumu }
        ...
        let puandetay = new genEmbed().setAuthor(uye.user.tag, uye.user.avatarURL({dynamic: true, size: 2048})).setThumbnail(uye.user.avatarURL({dynamic: true, size: 2048})).setDescription(`${uye} (${uye.roles.highest}) kullanıcısının şuan ki yetkisinde kazandığı puan bilgileri aşağıda belirtilmiştir.`)
...
${simdises}`},
        ...
        let genelpuandurumu = new genEmbed().setAuthor(uye.user.tag, uye.user.avatarURL({dynamic: true, size: 2048})).setThumbnail(uye.user.avatarURL({dynamic: true, size: 2048})).setDescription(`${uye} (${uye.roles.highest}) kullanıcısı <t:${Number(String(Date.parse(puanBilgisi.Baslama)).substring(0, 10))}:F> tarihinde yetkili olmuş, kazandığı toplam puanlar ve detayları aşağıda belirtilmiştir.`).addFields(
...
{ name: `Yetki Durumu`, value: `Şu an <@&${eskiRol.rol}> rolünden, son ${message.guild.roles.cache.get(_statSystem.endstaff) ? message.guild.roles.cache.get(_statSystem.endstaff) : "@Rol Bulunamadı."} rolüne ulaşabilmek için **\`${Number(ToplamPuan-puanBilgisi.ToplamPuan).toFixed(1)}\`** puan kazanmanız gerekiyor.`}
    ...
    if(görevBul) {
      ...
      if(görevBul.Time && görevBul.Time > 0 && Date.now() >= görevBul.Time) {
        ...
        yönetimPaneli = new genEmbed().setAuthor(uye.user.tag, uye.user.avatarURL({dynamic: true, size: 2048}))
    ...
    if(puanBilgisi && (görevBul || puanBilgisi.Yönetim)) return await message.reply({embeds: [yönetimPaneli], components: roller.kurucuRolleri.some(x => message.member.roles.cache.has(x)) || message.member.permissions.has("ADMINISTRATOR") ? [RowGorev] : [RowCukiki]}).then(async (manmsg) => {
      ...
      collector.on("collect", async (button) => {
        if(button.customId === "gorev_bilgisi") {
          let embedcik = new genEmbed().setAuthor(uye.user.tag, uye.user.avatarURL({dynamic: true, size: 2048})).setDescription(`Aşağı da ${görevBul.roleID ? message.guild.roles.cache.get(görevBul.roleID) : "@Rol Bulunamadı."} rolüne ait yapılması gereken görevler belirtilmiştir. Görev süreniz boyunca en az **%75**'lik kısmını tamamlamanız gerekmektedir.`)
