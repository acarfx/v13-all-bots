...
if(!Upstaffs ) {
  ...
  if(yetkiSalma) {
    if(yetkiSalma.unleashPoint >= 2 && !ayarlar.staff.includes(message.member.id)) {
      return message.reply({embeds: [new genEmbed().setFooter(`${yetkiSalma.unleashPoint} yetki salma hakkı bulunmakta.`).setDescription(`${message.guild.emojiGöster(emojiler.Iptal)} ${uye} isimli üyesi birden fazla kez yetki saldığından dolayı işlem yapılamıyor.`)]}).then(x => {
        setTimeout(() => {
          x.delete()
        }, 12500);
  ...
  msg = await message.reply({content: `Belirlenen **${uye.user.tag}** üyesi için işlem yükleniyor. Lütfen bekleyin!`})
  ...
  msg = await message.reply({content: `Belirlenen **${uye.user.tag}** üyesi için işlem yükleniyor. Lütfen bekleyin!`})
  await msg.edit({ content: null, embeds: [embed.setThumbnail(uye.user.avatarURL({dynamic: true})).setDescription(`Aşağıda ${uye} (\`${uye.id}\`) yetkilinin/yöneticinin analizleri detaylı bir şekilde belirtilmiştir.\n
**Yetkili Bilgileri**:
...
let collector = await msg.createMessageComponentCollector({filter, errors: ["time"], time: 45000 })
    ...
    collector.on("collect", async (button) => {
      if(button.customId === "bilgilendirme") {
          ...
          if (!res) return msg.edit({embeds: [new genEmbed().setAuthor(uye.user.tag, uye.user.avatarURL({dynamic: true})).setDescription(`${uye} isimli yetkilinin yükseltim geçmiş bilgisi bulunamadı.`)]}).then(x => setTimeout(() => {x.delete().catch(err => {})}, 7500)),button.deferUpdate().catch(err => {})
          if(!res.StaffLogs) return msg.edit({embeds: [new genEmbed().setAuthor(uye.user.tag, uye.user.avatarURL({dynamic: true})).setDescription(`${uye} isimli yetkilinin yükseltim geçmiş bilgisi bulunamadı.`)]}).then(x => setTimeout(() => {x.delete().catch(err => {})}, 7500)),button.deferUpdate().catch(err => {})
          let pages = res.StaffLogs.sort((a, b) => b.Date - a.Date).chunk(10);
          var currentPage = 1
          if (!pages && !pages.length || !pages[currentPage - 1]) return msg.edit({embeds: [new genEmbed().setAuthor(uye.user.tag, uye.user.avatarURL({dynamic: true})).setDescription(`${uye} isimli yetkilinin yükseltim geçmiş bilgisi bulunamadı.`)]}).then(x => setTimeout(() => {x.delete().catch(err => {})}, 7500)),button.deferUpdate().catch(err => {})
      ...
      if(button.customId == "puanver") {
        ...
        message.channel.send({embeds: [new genEmbed().setColor("YELLOW").setDescription(`${uye} isimli yetkiliye vermek veya almak istediğiniz yetki puanını belirtiniz.`)]}).then(async (mesaj) => {
          ...
          collector.on('collect', async (m) => {
            ...
            message.channel.send({embeds: [new genEmbed().setColor("WHITE").setDescription(`${uye} isimli yetkiliye \` ${puan} Yetki Puanı \` verilecek veya alınacak. Lütfen aşağıda ki düğmelerden seçiniz!`)], components: [Rowcuk]}).then(async (yarram) => {
