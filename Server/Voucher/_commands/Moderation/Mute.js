...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let msg = await message.reply({embeds: [new genEmbed().setAuthor(uye.user.tag, uye.user.displayAvatarURL({dynamic: true})).setDescription(`Belirtilen ${uye} isimli üyeyi hangi türde susturmak istiyorsun?`)], components: [muteOptions]}).catch(err => {})
    ...
    collector.on('collect', async i => {
        ...
        selectMute = 5
        i.update({embeds: [new genEmbed().setAuthor(uye.user.tag, uye.user.displayAvatarURL({dynamic: true})).setDescription(`Belirlenen ${uye} isimli üyesini hangi sebep ile **metin kanallarından** susturmamı istiyorsun?`)], components: [new MessageActionRow().addComponents(
        ...
        if (i.customId === `voicemute`) {
            selectMute = 4
            i.update({embeds: [new genEmbed().setAuthor(uye.user.tag, uye.user.displayAvatarURL({dynamic: true})).setDescription(`Belirlenen ${uye} isimli üyesini hangi sebep ile **ses kanallarından** susturmamı istiyorsun?`)], components: [new MessageActionRow().addComponents(
                new MessageSelectMenu()
                .setCustomId(`sebep`)
                .setPlaceholder(`Susturmak istediğiniz sebebi seçinin.`)
                .addOptions([
