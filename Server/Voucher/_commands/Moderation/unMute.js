...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    const sesmute = await Punitives.findOne({ Member: uye.id, Active: true, Type: "Ses Susturulma" })
        ...
        message.reply({components: [Row], embeds: [new genEmbed().setAuthor(message.member.user.tag, message.member.user.displayAvatarURL({dynamic: true}))
...
Belirtilen ${uye} üyesinin ${açıklama} kaldırmak için aşağıda ki düğmeleri kullanabilirsiniz.`)
                ...
                collector.on('collect', async (i) => {
                    if(i.customId == "metin") {
                        ...
                        if(!roller.muteHammer.some(oku => message.member.roles.cache.has(oku)) && !roller.üstYönetimRolleri.some(oku => message.member.roles.cache.has(oku)) && !roller.altYönetimRolleri.some(oku => message.member.roles.cache.has(oku)) && !roller.kurucuRolleri.some(oku => message.member.roles.cache.has(oku)) && !roller.yönetimRolleri.some(oku => message.member.roles.cache.has(oku))  && !message.member.permissions.has('ADMINISTRATOR')) return message.reply({content: `Belirtilen ${uye} isimli üyenin metin kanallarında ki susturmasını kaldırmak için yetkiniz yok. ${cevaplar.prefix}`}).then(x => {
                        ...
                        return i.deferUpdate().catch(err => {}),msg.delete().catch(err => {}),message.reply({embeds: [new genEmbed().setDescription(`${cevaplar.prefix} Bu ceza ${chatmute.Staff ? message.guild.members.cache.get(chatmute.Staff) ? `${message.guild.members.cache.get(chatmute.Staff)} (\`${chatmute.Staff}\`)` : `${chatmute.Staff}` :  `${chatmute.Staff}`} Tarafından cezalandırılmış. **Bu Cezayı Açman Münkün Değil!**`).setFooter("yaptırım yapılan cezada yaptırımı yapan yetkili işlem uygulayabilir.")]}).then(x => {
                            message.react(message.guild.emojiGöster(emojiler.Iptal) ? message.guild.emojiGöster(emojiler.Iptal).id : undefined)
                            setTimeout(() => {
                                x.delete()
                            }, 7500);
                    ...
                    if(i.customId == "ses") {
                        ...
                        return i.deferUpdate().catch(err => {}),msg.delete().catch(err => {}),message.reply({embeds: [new genEmbed().setDescription(`${cevaplar.prefix} Bu ceza ${sesmute.Staff ? message.guild.members.cache.get(sesmute.Staff) ? `${message.guild.members.cache.get(sesmute.Staff)} (\`${sesmute.Staff}\`)` : `${sesmute.Staff}` :  `${sesmute.Staff}`} Tarafından cezalandırılmış. **Bu Cezayı Açman Münkün Değil!**`).setFooter("yaptırım yapılan cezada yaptırımı yapan yetkili işlem uygulayabilir.")]}).then(x => {
                            message.react(message.guild.emojiGöster(emojiler.Iptal) ? message.guild.emojiGöster(emojiler.Iptal).id : undefined)
                            setTimeout(() => {
                                x.delete()
                            }, 7500);
