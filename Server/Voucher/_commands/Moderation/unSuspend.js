...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    if(uye) uye.send({embeds: [new genEmbed().setAuthor(uye.user.tag, uye.user.displayAvatarURL({dynamic: true})).setDescription(`${uye.user.tag}, ${message.author} tarafından <t:${String(Date.now()).slice(0, 10)}:R> şüpheliden çıkartıldın.`)]}).catch(x => {
