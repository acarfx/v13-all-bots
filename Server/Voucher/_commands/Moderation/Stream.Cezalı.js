...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let msg = await message.reply({embeds: [new genEmbed().setAuthor(uye.user.tag, uye.user.displayAvatarURL({dynamic: true})).setDescription(`Belirtilen ${uye} isimli üyeyi yayıncı odalarıda cezalandırmak istiyor musun?`)], components: [jailOptions]}).catch(err => {})
    ...
    collector.on('collect', async i => {
        if (i.customId === `onayla`) {
            i.update({embeds: [new genEmbed().setAuthor(uye.user.tag, uye.user.displayAvatarURL({dynamic: true})).setDescription(`Belirlenen ${uye} isimli üyesini hangi sebep ile yayıncı odalarından cezalandırmak istiyorsun?`)], components: [new MessageActionRow().addComponents(
