...
module.exports = {
    Isim: "kalkmazban",
    Komut: ["acarban", "uzaoç","forceban","xox","siktirgitamınoğlu"],
  ...
  onRequest: async function (client, message, args) {
    ...
    if(sunucudabul) {
        ...
        if(findedChannel) findedChannel.send({embeds: [new genEmbed().setFooter(`Ceza Numarası: #${cezano}`, uye.avatarURL({dynamic: true})).setDescription(`${uye.toString()} üyesine, <t:${String(Date.now()).slice(0, 10)}:R> \`${sebep}\` nedeniyle işlem uygulandı.`)]})
