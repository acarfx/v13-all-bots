...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    if(!roller.teyitciRolleri.some(oku => message.member.roles.cache.has(oku)) && !roller.kurucuRolleri.some(oku => message.member.roles.cache.has(oku)) && !message.member.permissions.has('ADMINISTRATOR')) return message.reply(cevaplar.noyt).then(s => setTimeout(() => s.delete().catch(err => {}), 5000));
...
${erkekTeyit+kizTeyit > 0 ? `Kayıt edilen bazı kişiler: ${bazıları ? bazıları.length > 10  ? `${bazıları.slice(0, 10).join(", ")} ve ${erkekTeyit + kizTeyit - 10} daha fazla ` : bazıları.slice(0, 10).join(",") : bazıları }`  : ''}`;
    ...
    message.reply({embeds: [new genEmbed().setAuthor(uye.user.tag, uye.user.avatarURL({dynamic: true})).setDescription(teyitBilgisi)]});
    }
};