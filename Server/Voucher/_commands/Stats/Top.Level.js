...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if(uye.user.tag.toString().length > 12) {
                image.setTextFont('18px Arial Black'), image.printText(`${uye.user.tag}`, 130, Number(start + 5), 850)
            } else {
                image.setTextFont('20px Arial Black'), image.printText(`${uye.user.tag}`, 130, Number(start + 2), 850)
            }
        ...
        let attachment = new MessageAttachment(image.toBuffer(), "acar-siralama.png");
