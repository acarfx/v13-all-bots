...
\`\`\``).setFooter(`${member.user.tag} • ${voiceChannel.name} • ${yayınSüresi}`, member.user.avatarURL({dynamic: true}))]}).catch(err => {})
  ...
  onRequest: async function (client, message, args) { 
    ...
    registerFont(`../../Assets/fonts/Acar.otf`,  { family: "Abc"});
    ...
    let load = await message.reply({content: `${message.guild.emojiGöster("948674910425853993")} | **${uye.user.tag}** isimli kullanıcının verileri yükleniyor. Lütfen bekleyin!`})
    let SonGörülme = await Seens.findOne({userID: uye.id})
    let data = await  Stats.findOne({ guildID: message.guild.id, userID: uye.id })
    let avatar = await loadImage(uye.user.displayAvatarURL({format: "png"}));
    ...
    if(roller.kadınRolleri && roller.kadınRolleri.some(x => uye.roles.cache.has(x))) background = await loadImage(`../../Assets/img/yatay_kadin.png`);
        ...
        if(data.lifeChatStats) {
          ...
          genelChatListe += `${message.guild.emojiGöster("acar_arrow")} ${message.guild.channels.cache.has(key) ? mesajkategoriismi ? mesajkategoriismi : message.guild.channels.cache.get(key).name : '#Silinmiş'}: \` ${value} mesaj \`\n`
        ...
        if(data.lifeVoiceStats) {
          ...
          if(_statSystem.voiceCategorys.find(x => x.id == key)) {
            ...
            let kategoriismi = kategori.isim 
              genelSesListe += `${message.guild.emojiGöster("acar_arrow")} ${message.guild.channels.cache.has(key) ? kategoriismi ? kategoriismi : `Diğer Odalar` : '#Silinmiş'}: \` ${client.sureCevir(value)} \`\n`  
            }
        
          });
          if(müzikToplam > 0) genelSesListe += `${message.guild.emojiGöster("acar_arrow")} Müzik Odalar: \` ${client.sureCevir(müzikToplam)} \`\n`
        }
        ...
        if(data.voiceStats) {
          ...
          if(_statSystem.voiceCategorys.find(x => x.id == key)) {
            let kategori = _statSystem.voiceCategorys.find(x => x.id == key);
            let kategoriismi = kategori.isim 
               haftalikSesListe += `${message.guild.emojiGöster("acar_arrow")} ${message.guild.channels.cache.has(key) ? kategoriismi ? kategoriismi : `Diğer Odalar` : '#Silinmiş'}: \` ${client.sureCevir(value)} \`\n`
          ...
          if(müzikToplam > 0) haftalikSesListe += `${message.guild.emojiGöster("acar_arrow")} Müzik Odalar: \` ${client.sureCevir(müzikToplam)} \``, canvasSesListe += `Müzik Odalar: ${client.sureCevir(müzikToplam)}`
        }
        ...
        let kategori = _statSystem.chatCategorys.find(x => x.id == key);
        let mesajkategoriismi = kategori.isim
        haftalikChatListe += `${message.guild.emojiGöster("acar_arrow")} ${message.guild.channels.cache.has(key) ? mesajkategoriismi ? mesajkategoriismi : message.guild.channels.cache.get(key).name : '#Silinmiş'}: \` ${value} mesaj \`\n`
        ...
        if(SonGörülme) {
          buttonGroup.push({label: "Son Görülme Detayları", description: `${uye.user.tag} üyesinin son görülme bilgileri gösterilmektedir.`, emoji: {id: "943290394949591070"}, value: "songorulme"})
        }
      if(ayarlar.statRozet && uye.id == message.member.id) {
        buttonGroup.push({label: "Rozet Bilgisi", description: `${uye.user.tag} üyesinin haftalık rozet bilgisini göstermektedir.`, emoji: {id: "1042968524559224942"}, value: "rozetcik"})
      }
      let sorumlu = await Sorumluluk.find({})
      sorumlu = sorumlu.filter(x => uye.roles.cache.has(x.role))
      if(sorumlu && sorumlu.length > 0) {
        buttonGroup.push([
          {label: "Sorumluluk Detayları", description: `${uye.user.tag} üyesinin liderlik ve sorumluluk bilgileri belirtilmektedir.`, emoji: {id: "1042968821855703081"}, value: "sorumluluklar"}
        ])
      }
      let meetingDedect = kullanicidata
      if(meetingDedect && meetingDedect.Meetings && meetingDedect.Meetings.length > 0) {
        buttonGroup.push([
          {label: "Toplantı Detayları", description: `${uye.user.tag} üyesinin son toplantı detayları belirtilmektedir.`, emoji: {id: "1042968943486320701"}, value: "toplantı"}
        ])
      }
      if(roller.Yetkiler.some(x => uye.roles.cache.has(x)) && Upstaffs) {
         buttonGroup.push([
           {label: `${Upstaffs.Yönetim ? "Görevleri Görüntüle" : "Yetkili İstatistikleri"}`, description: `${uye.user.tag} üyesinin ${Upstaffs.Yönetim ? "görevlerini görüntülemektedir." : "yetkili(yükseltim) istatistiklerini görüntülemektedir."}`, emoji: {id: `${Upstaffs.Yönetim ? "1042969210424406097" : "1042969515127996446"}`}, value: "panelaç"},
           {label: "Yükseltim Paneli", description: `${uye.user.tag} üyesinin yetki(yükselt/düşür) kontrolünü sağlar.`, emoji: {id: "621847216868425748" }, value: "buttonpanel"},
        ...
        let Rowcuk = new MessageActionRow().addComponents(
          new MessageSelectMenu()
            ...
            .setPlaceholder(`${uye.user.tag}'n detaylarını görüntüle`)
            .setOptions([
              buttonGroup
            ])
        )
       ...
       .setColor("#ffff")
       .setTextFont(`${String(uye.user.tag).length > 12 ? "38px" : "40px"} Bold`)
       .printText(`${uye.user.tag.replace("ş","s").replace("Ş","S").replace("İ","I").replace("ı","i").replace("Ç","C").replace("ç","c").replace("Ğ","G").replace("ğ","g").replace("Ö","O").replace("ö","ö")}`, String(uye.user.tag).length > 9 ? String(uye.user.tag).length > 12 ? 240 :  260 : 280 , 110, 350)
     let baslangic = 315
     let rozetler = []
     ...
     kart_resim.printText(`Tip: ${SonGörülme ? SonGörülme.last ? SonGörülme.last.type ? SonGörülme.last.type : `Bulunamadı!` : `Bulunamadı!` : "Bulunamadı!"}`, 80 * 4 + 45, 410 + 223, 200)
       ...
       let attachment = new MessageAttachment(kart_resim.toBuffer(), "acar-stat.png");
...
${genelChatListe ? genelChatListe ? genelChatListe : genelChatListe : `Daha önce bir listelenmiş mesaj istatistiği bulunamadı!`}`)
                  ...
                  collector.on('collect', async (i) => {
                    if(i.values[0] == "yayın") {
                      ...
                      msg.edit({embeds: [new genEmbed().setAuthor(uye.user.tag, uye.user.avatarURL({dynamic: true, size: 2048})).setThumbnail(uye.user.avatarURL({dynamic: true, size: 2048}))
...
${message.guild.emojiGöster("948684327959547965")} Toplam Yayın Süresi: **\` ${toplamYayın} \`**`).addField(`Son 20 Yayın Bilgisi  ${message.guild.emojiGöster("streamEmoji")}`, `${listedStreaming}`)], components: [Rowcuk]}).catch(err => {})
                  i.deferUpdate().catch(err => {})
                    ...
                    if(i.values[0] == "levelcik") {
                      ...
                      msg.edit({embeds: [new genEmbed().setAuthor(uye.user.tag, uye.user.avatarURL({dynamic: true, size: 2048}))
...
`)], components: [Rowcuk]}).catch(err => {})
                    ...
                    if(i.values[0] == "rozetcik") {
                      msg.edit({embeds: [embedcik.setAuthor(uye.user.tag, uye.user.avatarURL({dynamic: true, size: 2048}))
.setDescription(`${uye} (${uye.roles.highest}) üyesinin \`${message.guild.name}\` sunucusunda yaptığı haftalık aktifliğe göre uygun rozet bilgileri aşağıda belirtilmiştir.`)], components: [Rowcuk]}).catch(err => {})
                    ...
                    if(i.values[0] == "sorumluluklar") {
                      ...
                      msg.edit({embeds: [new genEmbed().setThumbnail(uye.user.avatarURL({dynamic: true, size: 2048})).setAuthor(uye.user.tag, uye.user.avatarURL({dynamic: true, size: 2048}))
                 ...
                 `)
                    ...
                    if(i.values[0] == "songorulme") {
                      ...
                      if(SonGörülme.last) {
                        ...
                        if(type == "ONLINE") txt = `${message.guild.emojiGöster("acar_arrow")} En son <t:${String(sonAktif).slice(0, 10)}:R> çevrim-içi oldu veya cihaz değiştirdi.`
                        if(type == "OFFLINE") txt = `${message.guild.emojiGöster("acar_arrow")} En son <t:${String(sonDeAktif).slice(0, 10)}:R> çevrim-dışı oldu.`
                        if(type == "VOICE") txt = `${message.guild.emojiGöster("acar_arrow")} En son <t:${String(sonSes).slice(0, 10)}:R> <#${SonGörülme.last.channel}> isimli ses kanalında görüldü.`
                        if(type == "MESSAGE") txt = `${message.guild.emojiGöster("acar_arrow")} Son etkinliği <t:${String(sonMesaj).slice(0, 10)}:R> <#${SonGörülme.last.channel}> isimli kanala \`${SonGörülme.last.text}\` mesajını gönderdi.`
                        if(type == "AVATAR") txt = `${message.guild.emojiGöster("acar_arrow")} Son etkinliği <t:${String(sonResim).slice(0, 10)}:R> profil resmini değiştirdi.`
                        if(type == "USERNAME") txt = `${message.guild.emojiGöster("acar_arrow")} Son etkinliği <t:${String(sonKullanıcıAdı).slice(0, 10)}:R> ${SonGörülme.last.old} olan kullanıcı adını ${SonGörülme.last.new} olarak değiştirdi.`
                        if(type == "DISCRIMINATOR") txt = `${message.guild.emojiGöster("acar_arrow")} Son etkinliği <t:${String(sonEtiket).slice(0, 10)}:R> #${SonGörülme.last.old} olan etiketini #${SonGörülme.last.new} olarak değiştirdi.`
                      }
...
${message.guild.emojiGöster("acar_arrow")} Son Görülme: <t:${String(SonGörülme.lastSeen).slice(0, 10)}:R> (\`${SonGörülme.last.type}\`)
${sonAktif ? `${message.guild.emojiGöster("acar_arrow")} Çevrim-İçi: <t:${String(sonAktif).slice(0, 10)}:R>` : `${message.guild.emojiGöster("acar_arrow")} Çevrim-İçi: ~`}
${sonDeAktif ? `${message.guild.emojiGöster("acar_arrow")} Çevrim-Dışı: <t:${String(sonDeAktif).slice(0, 10)}:R>` : `${message.guild.emojiGöster("acar_arrow")} Çevrim-Dışı: ~`}
${sonMesaj ? `${message.guild.emojiGöster("acar_arrow")} Sohbetde Görülme: <t:${String(sonMesaj).slice(0, 10)}:R>` : `${message.guild.emojiGöster("acar_arrow")} Sohbetde Görülme: ~`}
${sonSes ? `${message.guild.emojiGöster("acar_arrow")} Seste Görülme: <t:${String(sonSes).slice(0, 10)}:R>` : `${message.guild.emojiGöster("acar_arrow")} Seste Görülme: ~`}
${sonResim ? `${message.guild.emojiGöster("acar_arrow")} Resim Güncelleme: <t:${String(sonResim).slice(0, 10)}:R>` : `${message.guild.emojiGöster("acar_arrow")} Resim Güncelleme: ~`}
${sonKullanıcıAdı ? `${message.guild.emojiGöster("acar_arrow")} Kullanıcı Adı Güncelleme: <t:${String(sonKullanıcıAdı).slice(0, 10)}:R>` : `${message.guild.emojiGöster("acar_arrow")} Kullanıcı Adı Güncelleme: ~`}
${sonEtiket ? `${message.guild.emojiGöster("acar_arrow")} Etiket Güncelleme: <t:${String(sonEtiket).slice(0, 10)}:R>` : `${message.guild.emojiGöster("acar_arrow")} Etiket Güncelleme: ~`}
${txt ? txt : ''}`)
                        .setThumbnail(uye.user.avatarURL({dynamic: true, size: 2048}))
                    ...
                    if(i.values[0] == "haftalik") {
                      msg.edit({components: [Rowcuk],embeds: [new genEmbed().setAuthor(uye.user.tag, uye.user.avatarURL({dynamic: true, size: 2048}))
...
${haftalikChatListe ? haftalikChatListe ? haftalikChatListe : haftalikChatListe : `Daha önce kategorilendirilmiş haftalık mesaj istatistiği bulunamadı!`}`)
                        .setThumbnail(uye.user.avatarURL({dynamic: true, size: 2048}))]}).catch(err => {})
                      i.deferUpdate().catch(err => {})
                    }
