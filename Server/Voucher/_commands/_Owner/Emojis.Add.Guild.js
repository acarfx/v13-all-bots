...
module.exports = {
  ...
  onRequest: async function (client, msg, args) {
    ...
    if(!ayarlar.staff.includes(msg.member.id) && !msg.member.permissions.has('ADMINISTRATOR') && !roller.üstYönetimRolleri.some(oku => msg.member.roles.cache.has(oku)) && !roller.kurucuRolleri.some(oku => msg.member.roles.cache.has(oku))) return message.reply(cevaplar.noyt).then(s => setTimeout(() => s.delete().catch(err => {}), 5000));
