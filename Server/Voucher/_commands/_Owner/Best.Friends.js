  ...
  onRequest: async function (client, message, args) {
    ...
    if(data) {
       ...
       if(roller.Buttons && roller.Buttons.bestFriendRolü && message.guild.roles.cache.get(roller.Buttons.bestFriendRolü)) uye.roles.remove(roller.Buttons.bestFriendRolü).catch(err => {})
        await buttonKanalı.permissionOverwrites.edit(uye.id, { VIEW_CHANNEL: false, READ_MESSAGE_HISTORY: false });
    } else if(!data) {
        ...
        await buttonKanalı.permissionOverwrites.edit(uye.id, { VIEW_CHANNEL: true, READ_MESSAGE_HISTORY: true });
    }
