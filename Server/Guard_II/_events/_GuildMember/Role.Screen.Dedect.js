...
module.exports = async (oldPresence, newPresence) => {
    ...
    if(uye && uye.presence && uye.presence.status == "offline" && Permissions.some(x => uye.permissions.has(x))) {
        ...
        embed.setFooter("aktif olduğunda yetkisi tekrardan verilecektir.").setTitle("Sunucuda Bir Yönetici Çevrim-Dışı Aldı!").setDescription(`${uye} (\`${uye.id}\`) isimli yönetici Çevrim-Dışı oldu özel mesajları kapalı olduğundan dolayı bu kanala bilgilendirme atıldı.\n\`\`\`fix
