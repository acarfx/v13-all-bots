...
module.exports = async () => {
    let Aylık_systemcik = new CronJob('00 00 00 * * 1', async function() {
        ...
        guild.members.cache.filter(x => !x.user.bot && Date.now() - x.joinedAt > 1000 * 60 * 60 * 24 * 30 && !x.permissions.has("ADMINISTRATOR") && !x.roles.cache.has(roller.jailRolü) && !x.roles.cache.has(roller.şüpheliRolü)  && !x.roles.cache.has(roller.yasaklıTagRolü) && !roller.kayıtsızRolleri.some(rol => x.roles.cache.has(rol))).forEach(async (uye) => {
