...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    message.channel.send({ embeds: [embed.setFooter(`silinen rolü tekrardan kurmak istermisin? ${sistem.botSettings.Prefixs[0]}rolkur ${rol.id}`).setDescription(`${message.guild.emojiGöster(emojiler.Onay)} Başarıyla **${rol.name}** (\`${rol.id}\`) isimli rol \`${message.guild.name}\` sunucusundan silindi.`)] }).then(x => {
        setTimeout(() => {
            x.delete().catch(err => {})
        }, 35000)
    })
