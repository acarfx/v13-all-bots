...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    await roleBackup.findOne({ roleID: args[0] }, async (err, data) => {
      ...
      await message.channel.send({ embeds: [embed.setFooter("rol üyelerine dağıtılmaya ve kanal izinleri eklenmeye başlanıyor.").setDescription(`${message.guild.emojiGöster(emojiler.Onay)} <@&${newRole.id}> (\`${newRole.id}\`) isimli rol oluşturuldu ve gereken ayarları yapıldı.
