...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    message.channel.send({embeds: [new genEmbed().setDescription(`${message.guild.emojiGöster(emojiler.Onay)} Başarıyla **${tarihsel(Date.now() - ms("7d"))}** tarihinden itibaren olan tüm veriler temizlendi.`).setFooter("işlemi geri almak için bot yapımcısına başvurun.")]}).then(async (x) => {
