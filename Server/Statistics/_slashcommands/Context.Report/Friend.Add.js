    ...
    run: async (client, int, args) => {
        ...
        if(author_data && member_data && author_data.FollowUp.includes(member.id) && member_data.Follower.includes(author.id)) {
            ...
            if(member_data.FollowUp.includes(author.id)) {
                ...
                member.send(`${int.guild.emojiGöster(emojiler.Iptal)} **${author.user.tag}** isimli arkadaşınız sizi takipten **<t:${String(Date.now()).slice(0,10)}:R>** çıktı ve arkadaş listenizden kaldırıldı.`)
                .catch(err => {      
                })
                await Users.updateOne({_id: author.id}, {
                ...
                member.send(`${int.guild.emojiGöster(emojiler.Iptal)} **${author.user.tag}** sizi takipten **<t:${String(Date.now()).slice(0,10)}:R>** çıktı.`)
                .catch(err => {      
                })
            }
        } else {
            ...
            if(member_data && member_data.FollowUp.includes(author.id)) {
                ...
                member.send(`${int.guild.emojiGöster(emojiler.Onay)} **${author.user.tag}** ile birbirinizi **<t:${String(Date.now()).slice(0,10)}:R>** karşılıklı takip ederek arkadaş oldunuz.`)
                .catch(err => {
                ...
                member.send(`${int.guild.emojiGöster(emojiler.Iptal)} **${author.user.tag}** sizi **<t:${String(Date.now()).slice(0,10)}:R>** takip etmeye başladı.`)
                .catch(err => {
